...
.sidebar-bottom { margin-top: auto; padding-top: 12px; padding-bottom: 6px; }
...
def build_summary_text(model: Model) -> str:
    ...
    if deficits:
        ...
        for r, req, alloc, unalloc in deficits:
            ...
            lines.append(
                f"Περίοδος: {period}, Αρχικοί ΑΜ: {req}, Κατανεμημένοι ΑΜ: {alloc}, Μη κατανεμημένοι ΑΜ: {unalloc}"
            )
...
def process_excel(input_bytes: bytes) -> Tuple[bytes, str]:
    ...
    for r in list(model.row_period.keys()):
        if model.row_requested_am.get(r, 0) > 0 and model.row_x_count.get(r, 0) == 0:
            for ym in model.row_months_list.get(r, []):
                if ym in model.month_owner and model.month_owner[ym] != r:
                    if not make_month_free(model, ws, ym, depth=0, max_depth=6, visited=set()):
                        continue
                if ym in model.month_owner:
                    continue
    ...
    for (y, start_c, end_c) in model.year_blocks:
        if start_c != end_c:
...
with st.sidebar:
    ...
    st.link_button("📘 Οδηγίες χρήσης της εφαρμογής", GUIDE_DOC_URL, use_container_width=True)
    st.download_button(
